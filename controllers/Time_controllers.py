...
def create_time(time_data):
    ...
    if not all(key in time_data for key in ['titulo', 'Liga', 'Jogadores', 'tecnico']):
        response = make_response(
            json.dumps({'mensagem': 'Dados inválidos. titulo, Liga, Jogadores e tecnico são obrigatórios.'}, ensure_ascii=False),
    ...
    novo_time = time(     
        ...
        genero=time_data['Liga'],
        desenvolvedor=time_data['Jogadores'],
        plataforma=time_data['tecnico']
    )
...
def update_time(time_id, time_data):
    ...
    if not all(key in time_data for key in ['titulo', 'genero', 'tecnico', 'clube']):
        response = make_response(
            json.dumps({'mensagem': 'Dados invalidos. titulo, genero, tecnico e clube são abrigatórios'}, ensure_ascii=False),
    ...
    time.desenvolvedor = time_data['desenvolvedor']
    time.plataforma = time_data['plataforma']
